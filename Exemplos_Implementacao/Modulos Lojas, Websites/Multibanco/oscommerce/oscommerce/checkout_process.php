  ...
  if (is_object($$payment)) {
    ...
    if ($payment_class->email_footer) { 
      $email_order .= $payment_class->email_footer . "\n\n";
		...
		if($order->info['payment_method']=="Multibanco"){
			$email_order .= "ENTIDADE:   " . $payment_class->entidade."\n\n";
			$email_order .= "REFERÊNCIA: " . GenerateMbRef($payment_class->entidade,$payment_class->sub_entidade,$insert_id,$order_totals[sizeof($order_totals)-1]['value'])."\n\n";
			$email_order .= "VALOR:      " . $order_totals[sizeof($order_totals)-1]['text'];
